...
*/
HashMap_codes_t HashMap_name_init(HashMap_name *map, size_t size);
/**
 ...
 * HMERR_MEM - Memory allocation error.
*/
HashMap_codes_t HashMap_name_put(HashMap_name *map, const key__t *key, const val_t *value);
/**
...
*/
HashMap_codes_t HashMap_name_remove(HashMap_name *map, const key__t *key);
/**
 ...
 * HMERR_MEM - Memory allocation error.
*/
HashMap_codes_t HashMap_name_resize(HashMap_name *map, size_t new_size);
/**
...
HashMap_name * HashMap_name_snew(size_t size) {
  ...
  if (HashMap_name_init(map, size) != HM_SUCCESS) return NULL;
  return map;
}
...
HashMap_codes_t HashMap_name_put(HashMap_name *map, const key__t *key, const val_t *value) {
  ...
  map->size++;
  if (map->size == map->cap) {
    HashMap_codes_t res = HashMap_name_resize(map, map->cap + 1);
